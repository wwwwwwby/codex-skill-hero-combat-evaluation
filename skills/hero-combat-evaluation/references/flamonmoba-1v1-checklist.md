...
- Ambiguity handling:
  ...
  - if a hero has replacement skills, multi-stage casts, stance/form changes, or skill transformation mechanics, inspect local evidence first, then ask the author when needed; confirm whether the formula should read the current runtime ability from a skill slot before coding: lower/bottom slot is `Self.GetAbilityBySlot(2)`, middle slot is `Self.GetAbilityBySlot(1)`, upper/top slot is `Self.GetAbilityBySlot(0)`
...
- Ability data conventions:
  ...
  - if a hero uses runtime skill replacement, multi-stage casts, stance/form changes, or skill transformation mechanics, use `Self.GetAbilityBySlot(slot)` only after the relevant slot is confirmed by local evidence or by the author; the confirmed slot ability should be used for level, cooldown/readiness, bullet checks, and formulas that depend on the currently replaced skill
