...
- Ambiguity handling:
  ...
  - if a hero has replacement skills, multi-stage casts, stance/form changes, or skill transformation mechanics, inspect local evidence first, then ask the author when needed; first ask whether the hero has multi-stage/replacement-skill logic, then ask which skill is affected in author-facing terms: "下方、中间、上方中的哪一个技能" / lower, middle, or upper; also ask whether the document has fully accounted for that logic's combat-power impact before coding: lower/bottom slot uses `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot1)`, middle uses `BagSlot2`, upper/top uses `BagSlot3`; do not write raw slot indexes such as `2`, `1`, or `0`
...
- Ability data conventions:
  ...
  - if a hero uses runtime skill replacement, multi-stage casts, stance/form changes, or skill transformation mechanics, use slot access only after the relevant lower/middle/upper skill is confirmed by local evidence or by the author; when slot access is confirmed, call `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot1/2/3)`, never a raw slot index; the confirmed slot ability should be used for level, cooldown/readiness, bullet checks, and formulas that depend on the current slot ability
  - local evidence can confirm slot replacement when behavior-tree assets or existing code explicitly expose the slot, for example `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot*)` or `Check Slot With Skill Ability Type` with `Int32SlotIndex`; paired ability names such as `_close`, `_1`, or `_2` are only replacement hints unless paired with an explicit slot or CTO-reviewed implementation precedent
  - do not force slot access when the confirmed implementation needs a fixed authored ability; if the author, local code, or CTO-reviewed precedent confirms a specific `AbilityType` should be evaluated, use `Self.GetAbility(AbilityType.SomeSkill)` directly; if the slot/fixed-ability source is not confirmed, stop at Markdown review and ask
