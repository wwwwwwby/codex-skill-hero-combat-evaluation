...
- Do not read or use `MODIFIER_PROPERTY_WEAPON_DAMAGE_ADDITIONAL_FACTOR` / `MODIFIER_PROPERTY_ABILITY_TYPE_INJURY_DAMAGE_ADDITIONAL_FACTOR` in AI combat-evaluation code. If a hero needs basic-attack-specific or skill-specific damage additions, require the author to provide fixed constants in the confirmed Markdown and use those constants explicitly.
