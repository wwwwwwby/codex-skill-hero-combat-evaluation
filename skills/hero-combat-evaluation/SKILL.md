...
For replacement skills, multi-stage casts, stance/form changes, or any mechanic where a skill slot can hold different runtime abilities, explicitly ask the author whether the hero has multi-stage or replacement-skill logic before discussing implementation. When asking about the affected skill, use the author-facing phrasing "下方、中间、上方中的哪一个技能" / "which skill is it: lower, middle, or upper?" instead of asking for raw slot numbers first. Also ask the author to confirm whether the document has fully accounted for the combat-power impact of the multi-stage/replacement logic, including level, cooldown/readiness, damage, mobility, control, defense, resources, and any changed skill behavior. Confirm the slot before coding:
- Lower/bottom skill slot: `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot1)`
- Middle skill slot: `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot2)`
- Upper/top skill slot: `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot3)`
- Do not write raw slot indexes such as `2`, `1`, or `0` in new hero combat-evaluation code.
...
Once confirmed, use the slot ability for level and cooldown/readiness calculations only when the current runtime slot ability is the intended evaluation source, then pass that `Ability` through base helpers such as `GetAbilityLevel(...)` and `IsAbilityReadyWithin(...)`. If the author, local code, or CTO-reviewed precedent confirms that a fixed authored ability should be evaluated instead, use `Self.GetAbility(AbilityType.SomeSkill)` rather than forcing slot access. Do not use hand-written fallback ability-type lists.
...
- Avoid private `static` utility methods and static lookup arrays in hero classes. Hero-local lookup tables should be instance fields such as `private readonly float[]`, not `private static readonly float[]`. Formula helpers should normally be inlined when they only wrap one small formula, or moved to a base/shared helper when genuinely reusable.
...
| Hero has replacement, multi-stage cast, stance/form change, or skill transformation mechanics | Inspect local slot evidence first; if not conclusive, first ask the author whether the hero has multi-stage/replacement logic, then ask "下方、中间、上方中的哪一个技能" / lower, middle, or upper; also confirm the document fully accounts for the combat-power impact; if slot access is confirmed, use `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot1/2/3)` for level and readiness |
...
- Do not read or use `MODIFIER_PROPERTY_WEAPON_DAMAGE_ADDITIONAL_FACTOR` / `MODIFIER_PROPERTY_ABILITY_TYPE_INJURY_DAMAGE_ADDITIONAL_FACTOR` in AI combat-evaluation code. If a hero needs basic-attack-specific or skill-specific damage additions, require the author to provide fixed constants in the confirmed Markdown and write the numeric factors directly in the formula, such as `TotalWeaponDamage * 1.2f` in code or `攻击力 * 1.2` in Markdown. Do not reserve placeholder fields or variables for later replacement.
...
- Use shared AI math helpers from `RobotUtils` before calling `Mathf` in combat-evaluation code. Current project convention is `RobotUtils.Max/Min/Clamp/Clamp01/Lerp/Epsilon` for these operations in `RobotCombatAttributes` and the shared 1v1 calculation; leave APIs that do not yet exist in `RobotUtils` unchanged unless the project introduces a shared helper.
...
- Keep hero-local helpers narrow and formula-specific. Do not add private copies of base helpers, and do not add private `static` helper methods for formula decomposition. Inline tiny one-formula helpers when that keeps the refresh flow readable. Hero-local lookup arrays should be `private readonly`, not static.
- For heroes with runtime skill replacement, multi-stage casts, stance/form changes, or skill transformations, confirm the affected lower/middle/upper skill with the author before coding. If the current runtime slot ability is the intended evaluation source, use `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot1/2/3)` for the current runtime ability. Slot mapping is lower/bottom `BagSlot1`, middle `BagSlot2`, upper/top `BagSlot3`. Use that slot ability for `GetAbilityLevel`, cooldown readiness, bullet checks, and any formula that depends on the current slot ability.
- Local evidence can confirm the slot when behavior-tree assets or existing code explicitly expose it, for example `Self.GetAbilityBySlot((int)AbilitySlotType.BagSlot*)` or `Check Slot With Skill Ability Type` with `Int32SlotIndex`. Paired ability names such as `_close`, `_1`, or `_2` are only replacement hints unless paired with an explicit slot or CTO-reviewed implementation precedent.
- Do not force slot access when the confirmed implementation needs a fixed authored ability. If the author, local code, or CTO-reviewed precedent confirms a fixed ability such as a specific `_1` stage should be evaluated, use `Self.GetAbility(AbilityType.SomeSkill)` directly. If the document hints at multi-stage or replacement behavior but the slot/fixed-ability source is not confirmed, stop at Markdown review and ask.
- Hero classes that need `RobotSystem` should use the protected `RobotSystem` property from `RobotCombatAttributesBase`; do not call `SomeHelp.RobotSystem` or perform a local `Context.GetContext(...).GetService<IRobotSystem>()` lookup inside the hero class.
- Hero classes that evaluate summoned units should iterate `RobotSystem.Environment.AllSummoned`, filter by `SummonedType` and `Owner.Guid == Self.Guid`, and keep explicit summoned-state checks such as `IsDead`, `IsFakeDeath`, and callback state before treating the summoned unit as active.
